...
import net.packages.flying_machines.block.custom.AnimatedBlock;
import net.packages.flying_machines.block.custom.EnergyInjector;
...
public class Blocks {
                        ...
                        public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
                            if (itemsWithNoFunctional.contains(name)) {
                                if (Screen.hasShiftDown()) {
                                    tooltip.add(Text.translatable("tooltip.flying_machines.without_functional.detailed").formatted(Formatting.GRAY));
                                } else {
                                    tooltip.add(Text.translatable("tooltip.flying_machines.without_functional.hint").formatted(Formatting.RED));
                                }
                            }
                        }
            });
    ...
    public static void registerModBlocks() {
        flying_machines.LOGGER.info("Registering ModBlocks for " + flying_machines.MOD_ID);
    }
